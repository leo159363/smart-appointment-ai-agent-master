...
RAG 部分默认是轻量内置方案。课程知识存在 SQLite 的知识表中，启动时由 `KnowledgeService` 加载活跃知识，生成 embedding，并构建 FAISS 索引。用户咨询课程包、试听规则或老师推荐时，系统先检索相关知识，再组织回答。后续扩展上，我已经接入了 MODULAR-RAG-MCP-SERVER 的 Shadow 对比和可选 Primary 模式；默认仍是 local，Primary 失败时会回退本地 FAISS。
...
### 8.5 为什么 MODULAR-RAG-MCP-SERVER 先做 Eval-only/Shadow，再做可选 Primary？

当前本地 `KnowledgeService.search()` 已经支撑咨询 MVP。直接硬替换会影响咨询回答、PromptBuilder、页面演示和 API 行为。所以我先做 Eval-only：导出知识、设计 golden set；再做 Shadow：旁路对比检索结果；最后才做可选 Primary，并保留本地 FAISS fallback。这样能验证检索质量，同时保证 Modular 服务不可用时用户回答不受影响。
