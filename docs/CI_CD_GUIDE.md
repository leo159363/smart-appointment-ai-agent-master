...
`pytest` 当前只作为 CI/测试依赖安装，不代表生产运行必须依赖它。当前 CI 只执行 `pytest --collect-only`，用于验证测试可发现、包路径可导入。后续可以新增 `requirements-dev.txt` 或 `pyproject.toml` optional dependencies，统一管理测试依赖。
