...
- **主检索层（默认）**：通过 Modular RAG MCP Server（`primary` 模式）优先检索课程知识，支持 `mcp_stdio`/HTTP/CLI 三种 transport。
- **本地 fallback 层**：SQLite + Embedding + FAISS 向量索引，Modular 不可用时自动回退，也可通过 `RAG_MCP_MODE=local` 完全切回本地。
...
| RAG | Modular RAG MCP（主检索）、LangChain、FAISS、Embedding、课程知识库 |
...
Service 层
  ↓
Modular RAG MCP (primary) / SQLite / FAISS (fallback)
```
...
- `services/`：知识库（Modular RAG MCP + FAISS fallback）、老师、预约、用户行为、推荐和 Embedding 等业务服务。
...
- RAG 检索模式（可选）：默认 `RAG_MCP_MODE=primary`，优先走 Modular RAG MCP，失败时自动 fallback 到本地 FAISS；可设为 `local` 完全走本地，或 `shadow` 做旁路对比。
...
5. Modular RAG MCP Server 已作为默认 primary 主检索源接入，本地 FAISS 保留为 fallback。Modular 服务不可用时咨询链路自动降级，不影响本地演示。

以上边界不影响当前家教培训机构 MVP 的页面展示、API 演示和领域迁移说明。
