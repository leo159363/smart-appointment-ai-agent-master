...
async def initialize_system():
    """系统启动时自动初始化"""
    try:
        logger.info("🚀 正在初始化 AI 家教培训机构智能咨询与排课系统...")
        
        # 初始化知识库服务
        logger.info("📚 初始化课程知识库服务...")
        knowledge_service = KnowledgeService()
        await knowledge_service.initialize()
        
        # 初始化老师数据服务；内部仍沿用 TechnicianService 命名
        logger.info("👨‍🏫 初始化老师数据服务...")
...
def create_app() -> FastAPI:
    ...
    app = FastAPI(
        title="AI 家教培训机构智能咨询与排课系统",
        description="提供课程咨询、老师匹配、试听课预约、正式排课、知识库问答和学习需求分析等功能的 API 服务",
        version="1.0.0",
        docs_url="/docs",
        redoc_url="/redoc"
    )
